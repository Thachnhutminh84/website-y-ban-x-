...
echo "<h1>🔧 Áp dụng Menu Thống nhất - Chạy ngay</h1>";
echo "<p>Script này sẽ áp dụng menu đơn giản cho toàn bộ website</p>";

// Tạo backup
$backupDir = 'backup_menu_' . date('Y-m-d_H-i-s');
if (!is_dir($backupDir)) {
...
if (file_exists("auth.php")) require_once "auth.php";
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<body>
    ...
    .procedure-section { margin: 30px 0; }
    .procedure-grid {
        display: grid;
...
// Lấy danh sách file PHP
$allFiles = glob('*.php');
$excludeFiles = [
    'menu-don-gian.php',
    'apply-unified-menu-now.php',
    'sua-tat-ca-loi.php',
...
echo "<h2>📋 Xử lý " . count($phpFiles) . " file...</h2>";
...
foreach ($phpFiles as $file) {
    ...
    try {
        ...
        if (strpos($content, 'menu-don-gian.php') !== false) {
            echo "<p style='color: blue;'>⏭️ <strong>$file:</strong> Đã có menu đơn giản</p>";
            $skipCount++;
            continue;
        }
    ...
    <?php include 'menu-don-gian.php'; ?>", $content, 1);
        ...
        if ($replaced && $content !== $originalContent) {
            ...
            echo "<p style='color: green;'>✅ <strong>$file:</strong> Đã áp dụng menu đơn giản</p>";
            $successCount++;
        } else {
            echo "<p style='color: red;'>❌ <strong>$file:</strong> Không thể thêm menu</p>";
            $errorCount++;
        }
