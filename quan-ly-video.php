...
// Chỉ cán bộ và admin mới được truy cập
...
// Xử lý các action (chỉ khi không phải read-only)
$message = '';
$messageType = '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$isReadOnly) {
    ...
    try {
        ...
        if ($action == 'delete' && isset($_POST['video_id']) && authIsAdmin()) {
            ...
            $messageType = "success";
        } elseif ($action == 'delete' && !authIsAdmin()) {
            $message = "Bạn không có quyền xóa video!";
            $messageType = "error";
        }
