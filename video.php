...
try {
    ...
    if ($hasVideoTables) {
        ...
        if ($result && $result->num_rows > 0) {
            // Lấy danh sách album (loại bỏ trùng lặp)
            $stmt = $conn->prepare("SELECT DISTINCT id, name, description, is_active, display_order FROM video_albums WHERE is_active = 1 GROUP BY name ORDER BY display_order ASC, name ASC");
            $stmt->execute();
            $result = $stmt->get_result();
            $albumNames = []; // Mảng để track tên album đã thêm
            while ($row = $result->fetch_assoc()) {
                // Chỉ thêm album nếu tên chưa tồn tại
                if (!in_array($row['name'], $albumNames)) {
                    $albums[] = $row;
                    $albumNames[] = $row['name'];
                }
            }
...
// Hàm lấy video ID từ URL YouTube (hỗ trợ nhiều định dạng)
function getYouTubeVideoId($url) {
    ...
    $patterns = [
        ...
        '/youtube\.com\/watch\?.*v=([^&]+)/',         // https://www.youtube.com/watch?feature=...&v=VIDEO_ID
        '/youtube\.com\/shorts\/([^?]+)/',            // https://www.youtube.com/shorts/VIDEO_ID
    ];
    
    foreach ($patterns as $pattern) {
        if (preg_match($pattern, $url, $matches)) {
            // Lấy chỉ 11 ký tự đầu tiên (độ dài chuẩn của YouTube ID)
            return substr($matches[1], 0, 11);
        }
    }
    
    // Nếu không match pattern nào, kiểm tra xem có phải là video ID trực tiếp không
...
// Hàm kiểm tra loại video từ URL
