...
function generateVideoThumbnail($videoPath, $fileName) {
    ...
    // Sử dụng ffmpeg để tạo thumbnail (nếu có) - escapeshellarg để chống command injection
    $escapedVideoPath = escapeshellarg($videoPath);
    $escapedThumbnailPath = escapeshellarg($thumbnailPath);
    $ffmpegCmd = "ffmpeg -i $escapedVideoPath -ss 00:00:01 -vframes 1 -y $escapedThumbnailPath 2>/dev/null";
...
function getVideoDuration($videoPath) {
    $escapedVideoPath = escapeshellarg($videoPath);
    $ffprobeCmd = "ffprobe -v quiet -show_entries format=duration -of csv=p=0 $escapedVideoPath 2>/dev/null";
